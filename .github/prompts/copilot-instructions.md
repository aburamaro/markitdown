...
- `src/document_to_markdown/settings.py` は標準の入力・出力ディレクトリや対応拡張子を管理します。
...
- 設定値は `settings.py` に集約してください。
- CLI引数解析と変換処理は分離してください。
- Markdown変換処理と画像抽出処理は分離してください。
- PDF、Office、HTMLの画像抽出処理を1つの大きな関数にまとめないでください。
- 新しいファイル形式を変換対象に追加する場合は、`settings.py` の `SUPPORTED_EXTENSIONS` を更新してください。
